...
这里使用[[../../../../数学/基础/向量与图形/Hadamard乘积|Hadamard乘积]]来处理重置门和上一步的隐状态，可以减少以往状态的影响。显然，$\mathbf{R}_t \in (0,1)$，则当$\mathbf{R}_t$中的项接近1时，则取值全部依赖于$\mathbf{H}_{t-1}$，恢复为普通的循环神经网络，而如果其中的项接近0，候选隐状态是以$\mathbf{X}_t$作为输入的多层感知机的结果。